...
fn error_if_version_is_pruned(
    ...
    if let Some(pruner) = pruner.as_ref() {
        let least_readable_version =
            pruner.get_least_readable_version_by_pruner_index(pruner_index);
        ensure!(
            version >= least_readable_version,
            "{} version {} is pruned, least available version is {}.",
            data_type,
            version,
...
impl DbReader for AptosDB {
    ...
    /// Gets transaction by version, delegates to `AptosDB::get_transaction_with_proof`, returns an
    /// error if the provided version is not found.
