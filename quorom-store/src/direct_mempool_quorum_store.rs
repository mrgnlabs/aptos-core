...
async fn handle_consensus_request(
    ...
    let resp = ConsensusResponse::GetBlockResponse(Payload::DirectMempool(txns));
...
pub(crate) fn start_direct_mempool_quorum_store(
    executor: &Handle,
...
pub fn bootstrap(
    ...
    let runtime = Builder::new_multi_thread()
        .thread_name("direct-mempool-quorum-store")
        .enable_all()
        .build()
        .expect("[direct mempool quorum store] failed to create runtime");
    start_direct_mempool_quorum_store(
        runtime.handle(),
        consensus_receiver,
        mempool_sender,
